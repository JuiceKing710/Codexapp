...
def _resolve_session_for_phone(payload: PhoneLiveReadPayload):
    # Never auto-create a session from a poll payload — session must already exist
    # to prevent stale polls from contaminating a newly switched vehicle.
    if payload.session_id:
        return store.get_session(payload.session_id)
    if store.active_session_id:
        active = store.get_active_session()
        # Reject if the poll is for a different vehicle than the active session
        if payload.vehicle_id and active.vehicle_id != payload.vehicle_id:
            raise KeyError(f"vehicle_id mismatch: poll for {payload.vehicle_id} but active session is {active.vehicle_id}")
        return active
...
def create_session(payload: SessionCreateRequest | None = None) -> dict:
    ...
    if not vehicle_id:
        raise HTTPException(status_code=400, detail="vehicle_id is required to start a vehicle check")
    try:
        session = store.create_session(vehicle_id=vehicle_id)
    except KeyError:
...
function detectTransport(){
  ...
  const hasNativeBridge = Boolean(service && (typeof service.connectAdapter === 'function' || typeof service.connectToAdapter === 'function') && (typeof service.sendPIDCommand === 'function' || typeof service.readPid === 'function'));
...
async function readNativePid(sensor){
  ...
  try {
    const nativeRead = typeof service.sendPIDCommand === 'function'
      ? await service.sendPIDCommand(SENSOR_TO_PID[sensor])
...
function renderVehicles(){
  ...
  select.onchange = async (event) => {
    ...
    // Stop active session/connection before switching to prevent cross-vehicle data pollution
    if(state?.active_session){
      stopLivePolling();
      if((phoneBridge.status || 'disconnected') === 'connected'){
        try { await apiFetch('/phone/bridge/disconnect', { method: 'POST' }); } catch(_){}
      }
      try { await apiFetch('/sessions/active/stop', { method: 'POST' }); } catch(_){}
      phoneBridge = { status: 'disconnected', source_mode: 'PHONE-LIVE' };
    }
...
function renderBluetoothCard(){
  ...
  const detailRows = [
    ...
    { label: 'Native bridge detected', value: bridgeDebug.native_bridge_available ? 'Yes' : 'No' },
...
async function startLivePolling(){
  ...
  if(service?.startPolling){
    try {
      await service.startPolling({ intervalMs: 500, pids: LIVE_SENSOR_ORDER.map(sensor => SENSOR_TO_PID[sensor]).filter(Boolean) });
    } catch (error) {
      bridgeDebug.last_error = error?.message || 'native-start-polling-failed';
    }
  }
  logBridge('polling-armed',{interval:500});
  pollLiveSensorsOnce();
  livePollingHandle=setInterval(()=>{pollLiveSensorsOnce();},500);
}
async function stopLivePolling(){
  if(livePollingHandle){ clearInterval(livePollingHandle); livePollingHandle=null; }
  const service = getMobileBluetoothService();
  if(service?.stopPolling){
    try { await service.stopPolling(); } catch (_error) { }
  }
}
...
async function stopSession(){ await stopLivePolling(); await fetch('/sessions/active/stop',{method:'POST'}); await fetchState(); }
...
async function connectVehicle(){
  ...
  if(!transport.hasNativeBridge){
    ...
    try {
      ...
      const connectFn = transport.service.connectAdapter || transport.service.connectToAdapter;
      connectPayload = normalizeConnectPayload(await connectFn.call(transport.service), transport);
...
// ── Missing helpers required by active dashboard (defined only in duplicate block) ──
...
let transportContext = {
  ...
  message: 'Bluetooth is not available in this environment.',
  last_connection_error: null,
  first_pid_read_logged: false,
};

function emptyLocalConnectionDiagnostics(permissionStatus, nativeBridgeAvailable) {
  permissionStatus = permissionStatus || 'unknown';
  nativeBridgeAvailable = nativeBridgeAvailable || false;
  return {
...
function setLocalConnectionDiagnostics(updates) {
  updates = updates || {};
  localConnectionDiagnostics = Object.assign({}, localConnectionDiagnostics, updates);
  connectionDiagnostics = Object.assign({}, connectionDiagnostics, updates);
}

function resetLocalConnectionDiagnostics(opts) {
  opts = opts || {};
  const permissionStatus = opts.permissionStatus != null ? opts.permissionStatus : (localConnectionDiagnostics.bluetooth_permission_status || 'unknown');
  const nativeBridgeAvailable = opts.nativeBridgeAvailable != null ? opts.nativeBridgeAvailable : (localConnectionDiagnostics.native_bridge_available || false);
  localConnectionDiagnostics = emptyLocalConnectionDiagnostics(permissionStatus, nativeBridgeAvailable);
  connectionDiagnostics = Object.assign({}, localConnectionDiagnostics);
}

function setLastConnectionError(error) {
  const nextError = error || null;
  if (transportContext.last_connection_error !== nextError) {
    transportContext = Object.assign({}, transportContext, { last_connection_error: nextError });
  }
}

function markFirstPidRead(read) {
  if (!read || transportContext.first_pid_read_logged) { return; }
  transportContext = Object.assign({}, transportContext, { first_pid_read_logged: true });
  logBridge('first PID read', { pid: read.command, source_mode: read.source_mode });
}

function hasUsablePidRead(read) {
  if (!read) { return false; }
  const raw = read.raw_response || read.rawResponse;
  if (!raw || typeof raw !== 'string') { return false; }
  const upper = raw.trim().toUpperCase();
  if (upper === 'NO DATA' || upper === 'ERROR' || upper === '?' || upper.startsWith('UNABLE')) { return false; }
  return Boolean(read.value !== null && read.value !== undefined && read.value !== '');
}

function transportServiceReady() {
  const t = detectTransport();
  if ((phoneBridge.status || 'disconnected') !== 'connected') { return false; }
  if (t.hasNativeBridge) { return Boolean(getMobileBluetoothService()); }
  return false;
}

function handleNativePollEvent(event) {
  const nativeRead = event && event.detail ? event.detail : null;
  if (!nativeRead || !hasUsablePidRead(nativeRead)) { return; }
  setLocalConnectionDiagnostics({ first_pid_command_sent: true, first_pid_response_received: true });
...
async function handleNativeDisconnectEvent(event) {
  ...
  phoneBridge = Object.assign({}, phoneBridge, detail, { status: 'disconnected', last_error: reason, fallback_reason: reason });
  stopLivePolling();
  renderDashboardState();
  renderBluetoothCard();
  try { await publishBridgeDisconnect(); } catch (_) {}
  await fetchState();
}
...
function transportLog(event, extra) {
  extra = extra || {};
  console.debug('[transport]', Object.assign({ event }, extra));
}

function formatDiagnosticValue(value) {
  if (typeof value === 'boolean') { return value ? 'true' : 'false'; }
  return String(value != null ? value : 'unknown');
}
